...
class AzureCognitiveSearchDocumentUploadStrategy(DocumentUploadStrategy):
...
class ChromaDBDocumentUploadStrategy(DocumentUploadStrategy):
    """Upload strategy for ChromaDB with local embeddings."""
    
    def __init__(self, processing_strategy: DocumentProcessingStrategy=None):
        """
        Initialize the ChromaDB upload strategy.
        
        Automatically loads configuration from environment variables:
        - CHROMADB_COLLECTION_NAME (optional, defaults to 'documentation_collection')
        - CHROMADB_PERSIST_DIRECTORY (optional, defaults to './chromadb_data')
        
        Args:
            processing_strategy: Processing strategy to use for creating search objects
        """
        # Load configuration from environment variables
        self.collection_name = os.getenv('CHROMADB_COLLECTION_NAME', 'documentation_collection')
    ...
    def chromadb_service(self):
        """Lazy initialization of ChromaDB service."""
    ...
    async def upload_documents(self, processed_documents: List[ProcessedDocument], 
        ...
        This method processes each document individually:
        1. Creates ChromaDB search objects with embeddings for each document
        2. Uploads the objects to ChromaDB immediately
        3. Marks successfully uploaded files as processed in the tracker immediately
        
        Args:
            processed_documents: List of processed documents from processing phase
            tracker: DocumentProcessingTracker instance for marking files as processed
            **kwargs: Additional parameters (unused for ChromaDB strategy)
        ...
        if not self.chromadb_service.test_connection():
            error_msg = "ChromaDB connection failed - cannot proceed with upload"
            errors.append(error_msg)
            logger.error(error_msg)
            print(f"❌ {error_msg}")
            ...
            return DocumentUploadResult(
                ...
                errors=errors,
                strategy_name=self.strategy_name,
                upload_metadata={"connection_failed": True}
            )
        ...
        for doc_idx, processed_doc in enumerate(processed_documents, 1):
            try:
                ...
                if doc_search_objects:
                    # Upload this document's search objects to ChromaDB
                    print(f"      📤 Uploading {doc_total_objects} objects to ChromaDB...", end=" ")
                    
                    successful, failed = self.chromadb_service.upload_search_objects_batch(doc_search_objects)
    ...
    async def delete_all_documents_from_service(self) -> int:
        """
        Delete all documents from ChromaDB service.
        
        Returns:
            Number of documents deleted from the service
        """
        try:
            logger.info("Deleting all documents from ChromaDB service...")
            print(f"   🗑️  Deleting all documents from ChromaDB service...")
            ...
            # ChromaDB doesn't have a direct "delete all" method, so we need to get all document IDs first
            # Get all document IDs in batches
            all_results = self.chromadb_service.collection.get()
            all_ids = all_results.get('ids', [])
            
            if all_ids:
                # Delete all documents by ID
                self.chromadb_service.collection.delete(ids=all_ids)
